...
public class S21PacketChunkData extends Packet
{
	 ...
	 * Extracted builder
	 * */
	public static S21PacketChunkData.Extracted func_149269_a(Chunk exporter, boolean sendClient, int updateMarker) {
		Extracted result = new Extracted();
		ExtendedBlockStorage[] arr = exporter.getBlockStorageArray();
		byte[] tmp = field_149286_i;
		int count = 0;
		
		//extract chunk segments data
		for(int i = 0, l = arr.length; i < l; ++i) {
			if(arr[i] != null && (!sendClient || !arr[i].isEmpty()) && (updateMarker & 1 << i) != 0) {
				byte[] data = arr[i].exportPalette();
				
				System.arraycopy(data, 0, tmp, count, data.length);
				count += data.length;
				result.field_150280_b |= 1 << i;
			}
		}
		
		//extract other
		if(sendClient) {
			exporter.sendUpdates = true;
			
			byte[] biome = exporter.getBiomeArray();
			System.arraycopy(biome, 0, tmp, count, biome.length);
			count += biome.length;
		}
		
		//return the result
		System.arraycopy(tmp, 0, result.field_150282_a = new byte[count], 0, count);
		return result;
	}
    //XXX
