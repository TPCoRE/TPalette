...
public class Chunk
{
	 ...
	 */
	public void fillChunk(byte[] data, int updateMarker, int noused, boolean biome) {
		ExtendedBlockStorage[] arr = this.storageArrays;
		boolean skyl = !this.worldObj.provider.hasNoSky;
		int count = 0;
		
		//accept segments data
		for(int i = 0, l = arr.length; i < l; ++i) {
			if((updateMarker & 1 << i) != 0) { //'<<'运算符的优先级比'&'高，所以没事
				ExtendedBlockStorage segment = arr[i];
				if(segment == null) segment = arr[i] = new ExtendedBlockStorage(i << 4, skyl);
				
				count += segment.acceptPalette(data, count);
				segment.removeInvalidBlocks();
			}
		}
		
		//accept biomes data
		if(biome) System.arraycopy(data, count, this.blockBiomeArray, 0, this.blockBiomeArray.length);
		
		//update/init data
		this.isLightPopulated = true;
		this.isTerrainPopulated = true;
		this.generateHeightMap();
		Iterator iter = this.chunkTileEntityMap.values().iterator();
		//update tileEntities info
		while(iter.hasNext()) ((TileEntity) iter.next()).updateContainingBlockInfo();
	}
	
	/**
	 * See {@link ExtendedBlockStorage#peekTag(int, int, int, NBTTagCompound) peekTag}
	 * */
	public boolean peekTag(int x, int y, int z, NBTTagCompound peeker) {
		ExtendedBlockStorage segment = this.storageArrays[y >> 4];
		return segment == null ? false : segment.peekTag(x, y & 15, z, peeker);
	}
	
	/**
	 * Get tag, the copy of, null for non-existed
	 * */
	public NBTTagCompound getTag(int x, int y, int z) {
		ExtendedBlockStorage segment = this.storageArrays[y >> 4];
		return segment == null ? null : segment.getTag(x, y & 15, z);
	}
	
	/**
	 * Set tag, null for deleting, return itself
	 * */
	public Chunk setTag(int x, int y, int z, NBTTagCompound tag) {
		ExtendedBlockStorage[] segments = this.storageArrays;
		if(segments[y >> 4] == null) {
			if(tag == null) return this;
			segments[y >> 4] = new ExtendedBlockStorage(y >> 4 << 4, !this.worldObj.provider.hasNoSky);
		}
		
		segments[y >> 4].setTag(x, y & 15, z, tag);
		return this;
	}
	//XXX
